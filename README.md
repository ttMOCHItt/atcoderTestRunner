...
`atcheck`実行時にサンプルが1件も取得できなかった場合、自動でログインするか確認されます。`y`と答えると、AtCoderのセッションCookie（`REVEL_SESSION`）の入力を求められます。

ユーザー名・パスワードでの自動ログインはAtCoder側のボット対策で弾かれるため非対応です。代わりに、先にブラウザでAtCoderにログインしておき、そのセッションCookieを1回だけ貼り付ける方式になっています。

**Cookieの取得方法**

1. ブラウザでAtCoderにログインする
2. 開発者ツール（F12）を開く
3. Application（Chrome）または Storage（Firefox）タブ → Cookies → `https://atcoder.jp`
4. `REVEL_SESSION` の値をコピー
5. `atcheck`の「REVEL_SESSION: 」というプロンプトに貼り付け（非表示入力）

- ログイン成功後はセッション情報が`~/.atcoder_session.pkl`に保存され、次回以降は再ログイン不要です。
- 共有PCなど他人がログインする場合や、セッションを無効化したい場合は、このファイルを削除してください。
